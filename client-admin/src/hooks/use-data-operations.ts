...
export function useDataOperations<T extends Record<string, unknown>>({
  ...
  const deleteItem = useCallback((id: number | string) => {
    setData(prev => prev.filter(item => String(item[idField]) !== String(id)));
    setFilteredData(prev => prev.filter(item => String(item[idField]) !== String(id)));
  toast.success("Xóa mục thành công");
  }, [idField]);

  const bulkDelete = useCallback((ids: (number | string)[]) => {
    setData(prev => prev.filter(item => !ids.includes(String(item[idField]))));
    setFilteredData(prev => prev.filter(item => !ids.includes(String(item[idField]))));
  ...
  const handleSearch = useCallback((term: string) => {
    ...
    if (term) {
      filtered = searchInData(filtered, term, searchFields) as T[];
    }
  ...
  const handleDateRangeChange = useCallback((range: DateRange | undefined) => {
    ...
    // Apply search filter if term is provided
    if (searchTerm) {
      filtered = searchInData(filtered, searchTerm, searchFields) as T[];
    }
    ...
    if (range?.from || range?.to) {
      ...
      filtered = searchTerm ? searchInData(data, searchTerm, searchFields) as T[] : data;
    }
  ...
  const bulkUpdate = useCallback((ids: (number | string)[], updates: Partial<T>) => {
    setData(prev => 
      prev.map(item => 
        ids.includes(String(item[idField])) ? { ...item, ...updates } : item
      )
    );
